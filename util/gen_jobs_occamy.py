...
def get_xy_base_addr(x: int, y: int):
    ...
    return (MEM_BASE + ((x-1) + 2**clog2(NUM_X)*(y-1))*MEM_SIZE)

def get_hbm_base_addr():
    return HBM_BASE
...
def gen_mesh_traffic(narrow_burst_length: int,
    ...
    for x in range(1, NUM_X+1):
        for y in range(1, NUM_Y+1):
            ...
            assert (wide_length <= MEM_SIZE and narrow_length <= MEM_SIZE)
            if type == 'hbm':
                # Tile x=0 y=1 is our occamy_top
                # Each core read from this top
                # hbm_addr = get_xy_base_addr(0, 1)
                hbm_addr = get_hbm_base_addr()
